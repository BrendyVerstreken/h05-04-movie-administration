    ...
    public class MoviesController : Controller
    {
        ...
        public async Task<IActionResult> Add(MoviesAddUpdateMovieViewModel moviesAddMovieViewModel)
        {
            ...
            if (moviesAddMovieViewModel.Image != null)
            {
                ...
                newMovie.ImageFileName = await _fileService.StoreFile(moviesAddMovieViewModel.Image,
                    "movies");
            }
            //store actors
        ...
        public async Task<IActionResult> Edit(long id)
        {
            ...
            //get the actors and check isselected if they belong to movie
            //complex Linq code, not very Db efficient
            //moviesUpdateMovieViewModel.Actors =
            //    await _movieContext
            //    .Actors
            //    .Select(a => new CheckboxHelper
            //    {
            //        Text = $"{a.FirstName} {a.LastName}",
            //        Id = a.Id,
            //        IsSelected = _movieContext.Movies.Include(m => m.Actors).FirstOrDefault(m => m.Id == movie.Id).Actors.Any(ac => ac.Id == a.Id)
            //    }).ToListAsync();
            ////directors
            //moviesUpdateMovieViewModel.Directors
            //    = await _movieContext
            //    .Directors
            //    .Select(d => new CheckboxHelper
            //    {
            //        Id = d.Id,
            //        Text = $"{d.FirstName} {d.LastName}",
            //        IsSelected = _movieContext.Movies.Include(m => m.Directors).FirstOrDefault(m => m.Id == movie.Id).Directors.Any(di => di.Id == d.Id)
            //    }).ToListAsync();
            //old skool code, less hassle on the database
            //actors
            moviesUpdateMovieViewModel.Actors
        ...
        public async Task<IActionResult> Edit(MoviesAddUpdateMovieViewModel moviesUpdateMovieViewModel)
        {
            ...
            if(moviesUpdateMovieViewModel.Image != null)
            {
                ...
                {
                    movie.ImageFileName = await _fileService.StoreFile(moviesUpdateMovieViewModel.Image,
                        "movies");
                }
                else
                {
                    movie.ImageFileName = await _fileService
                        .StoreFile(moviesUpdateMovieViewModel.Image, "movies", movie.ImageFileName);
                }
            }
            //save
            await _movieContext.SaveChangesAsync();
